let update = function(){
	g("e1").innerText = Serotonin.amount().format(true);
	g("e2").innerText = Dopamine.amount().format(true);
	g("e3").innerText = Happiness.amount().format(true);
	g("e4").innerText = Serotonin.amount().format(true);
	g("e5").innerText = Serotonin.gainAmount().format(true);
	g("e6").innerText = Dopamine.amount().format(true);
	g("e7").innerText = Dopamine.gainAmount().format(true);
	g("e8").innerText = Happiness.amount().format(true);
	g("e9").innerText = Happiness.gainAmount().format(true);
	g("e10").innerText = Happiness.serotoninRequirement().format(true);
	g("e11").innerText = Happiness.dopamineRequirement().format(true);
	g("e12").innerText = Happiness.gainAmount().format(true);
	g("e13").innerText = Dopamine.gainAmount().format(true);
	g("e14").innerText = Happiness.buttonAmount().format(true);
	g("e15").style.backgroundColor = Happiness.canGain() ? '#aaa' : '#444';
	g("e16").innerText = Upgrade(0).currentEffect().format(true);
	g("e17").innerText = Upgrade(0).nextEffect().format(true);
	g("e18").innerText = Upgrade(0).cost().format(true);
	g("e19").innerText = Upgrade(1).currentEffect().format(true);
	g("e20").innerText = Upgrade(1).nextEffect().format(true);
	g("e21").innerText = Upgrade(1).cost().format(true);
	g("e22").innerText = Upgrade(2).currentEffect().format(true);
	g("e23").innerText = Upgrade(2).nextEffect().format(true);
	g("e24").innerText = Upgrade(2).cost().format(true);
	g("e25").style.backgroundColor = Upgrade(0).canBuy() ? '#aaa' : '#444';
	g("e26").style.backgroundColor = Upgrade(1).canBuy() ? '#aaa' : '#444';
	g("e27").style.backgroundColor = Upgrade(2).canBuy() ? '#aaa' : '#444';
	g("e28").innerText = Upgrade(0).buttonText();
	g("e29").innerText = Upgrade(1).buttonText();
	g("e30").innerText = Upgrade(2).buttonText();
	g("e31").style.backgroundColor = Upgrade(3).canBuy() ? '#aaa' : '#444';
	g("e32").innerText = Upgrade(3).currentEffect().format(true);
	g("e33").innerText = Upgrade(3).nextEffect().format(true);
	g("e34").innerText = new Decimal(Upgrade(3).cost()).format(true);
	g("e35").innerText = Upgrade(3).buttonText();
	g("e36").style.backgroundColor = Upgrade(4).canBuy() ? '#aaa' : '#444';
	g("e37").innerText = Upgrade(4).currentEffect().format(true);
	g("e38").innerText = Upgrade(4).nextEffect().format(true);
	g("e39").innerText = new Decimal(Upgrade(4).cost()).format(true);
	g("e40").innerText = Upgrade(4).buttonText();
	g("e41").style.backgroundColor = Upgrade(5).canBuy() ? '#aaa' : '#444';
	g("e42").innerText = Upgrade(5).currentEffect();
	g("e43").innerText = Upgrade(5).nextEffect();
	g("e44").innerText = new Decimal(Upgrade(5).cost()).format(true);
	g("e45").innerText = Upgrade(5).buttonText();
	g("e46").style.backgroundColor = Upgrade(6).canBuy() ? '#aaa' : '#444';
	g("e47").innerText = Upgrade(6).currentEffect();
	g("e48").innerText = Upgrade(6).nextEffect();
	g("e49").innerText = new Decimal(Upgrade(6).cost()).format(true);
	g("e50").innerText = Upgrade(6).buttonText();
	g("e51").innerText = Serotonin.netPerSecond().format(true);
	g("e52").innerText = Dopamine.netPerSecond().format(true);
	g("e53").innerText = Happiness.netPerSecond().format(true);
	g("e54").style.display = Upgrade(6).level() == 0 ? 'none' : 'inline-block';
	g("e55").style.display = Dopamine.netPerSecond().eq(0) ? 'none' : 'inline-block';
	g("e56").style.display = Upgrade(6).level() == 0 ? 'none' : 'inline-block';
	g("e57").innerText = Happiness.produceable().format(true);
	g("e58").style.display = Happiness.shouldDisplayProduceable() ? 'block' : 'none';
	g("e59").style.display = Upgrade(3).atMaxLevel() && Upgrade(4).atMaxLevel() ? 'none' : 'inline-block';
	g("e60").style.display = Upgrade(3).atMaxLevel() && Upgrade(4).atMaxLevel() ? 'none' : 'inline-block';
	g("e61").style.display = Prestige.canSee() ? 'inline-block' : 'none';
	g("e62").style.display = Emotion.canSee() ? 'block' : 'none';
	g("e63").innerText = Emotion.amount().format(true);
	g("e64").style.display = Prestige.canGain() ? 'inline-block' : 'none';
	g("e65").innerText = Prestige.gain().format(true);
	g("e66").innerText = Prestige.timesTotal().format(true);
	g("e67").innerText = Prestige.perSecond().format(true);
	g("e68").innerText = Emotion.multiplier().format(true);
	g("e69").innerText = timeFormat(player.stats.timeInExperience);
	g("e70").innerText = Experiences.amount().format(true);
	g("e71").innerText = Experiences.multiplier().format(true);
	g("e72").innerText = ExperienceUpgrade(0).effect().format(true);
	g("e73").innerText = ExperienceUpgrade(0).nextEffect().format(true);
	g("e74").innerText = ExperienceUpgrade(0).cost().format(true);
	g("e75").innerText = ExperienceUpgrade(0).level();
	g("e76").innerText = ExperienceUpgrade(1).effect().format(true);
	g("e77").innerText = ExperienceUpgrade(1).nextEffect().format(true);
	g("e78").innerText = ExperienceUpgrade(1).cost().format(true);
	g("e79").innerText = ExperienceUpgrade(1).level();
	g("e80").innerText = ExperienceUpgrade(2).effect();
	g("e81").innerText = ExperienceUpgrade(2).nextEffect();
	g("e82").innerText = ExperienceUpgrade(2).cost().format(true);
	g("e83").innerText = ExperienceUpgrade(2).level();
	g("e84").innerText = ExperienceUpgrades.totalLevel();
	g("e85").innerText = ExperienceUpgrades.totalLevelMultiplier().format(true);
	g("e86").style.backgroundColor = ExperienceUpgrade(0).canBuy() ? '#aaa' : '#444';
	g("e87").style.backgroundColor = ExperienceUpgrade(1).canBuy() ? '#aaa' : '#444';
	g("e88").style.backgroundColor = ExperienceUpgrade(2).canBuy() ? '#aaa' : '#444';

	g("e25").style.display = Upgrade(0).atMaxLevel() ? 'none' : 'inline-block';
	g("e26").style.display = Upgrade(1).atMaxLevel() ? 'none' : 'inline-block';
	g("e27").style.display = Upgrade(2).atMaxLevel() ? 'none' : 'inline-block';
	g("e31").style.display = Upgrade(3).atMaxLevel() ? 'none' : 'inline-block';
	g("e36").style.display = Upgrade(4).atMaxLevel() ? 'none' : 'inline-block';
	g("e41").style.display = Upgrade(5).atMaxLevel() ? 'none' : 'inline-block';
	g("e46").style.display = Upgrade(6).atMaxLevel() ? 'none' : 'inline-block';
	g("e89").style.display = Happiness.shouldDisplayAmounts() ? 'block' : 'none';

	g("btn-experiences").style.display = player.experiences.gt(0) ? 'inline-block' : 'none';
};